...
impl<B: ByteSliceMut> Branch<B> {
    pub fn initialize(&mut self, key: &[u8], left_child: PageId, right_child: PageId) {
        ...
        self.insert(0, key, left_child).unwrap();
...
mod tests {
    ...
    fn test_split() {
        ...
        let mut new_branch = Branch::new(new_data.as_mut_slice());
        let split_key = branch.split_insert(&mut new_branch, &10u64.to_be_bytes(), PageId(5));
        assert_eq!(&8u64.to_be_bytes(), split_key.as_slice());

        assert_eq!(2, branch.num_pairs());
        assert_eq!(1, new_branch.num_pairs());

        assert_eq!(PageId(1), new_branch.search_child(&1u64.to_be_bytes()));
        assert_eq!(PageId(3), new_branch.search_child(&5u64.to_be_bytes()));
        assert_eq!(PageId(3), new_branch.search_child(&6u64.to_be_bytes()));
